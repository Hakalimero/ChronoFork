...
#define BTN_UP    33
#define BTN_DOWN  25
#define BTN_BACK  21  // Ajout du bouton retour
...
void handleRoot() {
  ...
  html += ".item{display:block;padding:12px;margin:5px 0;background:white;border-radius:5px;text-decoration:none;color:#333;box-shadow:0 1px 2px rgba(0,0,0,0.1);}";
  html += ".dir{border-left:6px solid #ffc107;font-weight:bold;} .file{border-left:6px solid #007bff;}";
  ...
  while(file){
    ...
    if (file.isDirectory()) html += "<a class='item dir' href='/?dir=" + fullPath + "'>📁 " + name + "</a>";
    else html += "<a class='item file' href='/download?file=" + fullPath + "'>📄 " + name + "</a>";
    file = root.openNextFile();
  }
  html += "<hr><h4>Upload Circuit (.json)</h4><form method='POST' action='/upload' enctype='multipart/form-data'><input type='file' name='f'><input type='submit' value='Envoyer'></form></body></html>";
...
void handleDownload() {
  ...
  if (SD.exists(path)) {
    ...
    String dataType = (path.endsWith(".csv")) ? "text/csv" : "application/octet-stream";
    server.streamFile(f, dataType);
    f.close();
  } else server.send(404, "Inexistant");
}
...
void handleFileUpload() {
  ...
  else if (upload.status == UPLOAD_FILE_END && fsUploadFile) { fsUploadFile.close(); server.sendHeader("Location", "/"); server.send(303); }
}
...
void setup() {
  ...
  pinMode(BTN_BACK, INPUT_PULLUP); // Initialisation Pin 21
...
void loop() {
  ...
  if (currentMode == MENU) {
    ...
    u8g2.drawStr(10, 45, "COURSE EN COURS...");
    ...
    u8g2.setCursor(10, 85); u8g2.print("IP: " + WiFi.softAPIP().toString());
  }
  else if (currentMode == CIRCUIT_SEL) {
    u8g2.setFont(u8g2_font_9x15_tf);
    u8g2.drawStr(10, 35, "LISTE CIRCUITS");
  ...
  if (digitalRead(BTN_OK) == LOW) {
    ...
    if (currentMode == MENU) {
      ...
      if (currentMode == WIFI_MODE) {
        ...
        server.on("/", handleRoot); server.on("/download", handleDownload);
